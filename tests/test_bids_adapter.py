...
def test_read_bids_eyetrack_maps_required_columns_and_metadata(tmp_path):
    ...
    np.testing.assert_allclose(
        record.study.data["timestamp"], [1000.0, 1010.0, 1020.0]
    )
...
def test_read_bids_eyetrack_converts_seconds_and_can_drop_extra_columns(tmp_path):
...
def test_bids_reader_rejects_bad_json_empty_data_and_missing_subject(tmp_path):
    path, sidecar = _write_record(
        tmp_path,
        "sub-01_task-demo_recording-eye1_physio.tsv.gz",
    )
    ...
    empty_path, _ = _write_record(
        tmp_path,
        "sub-02_task-demo_recording-eye1_physio.tsv",
        rows=[],
    )
    with pytest.raises(ValueError, match="at least one row"):
        read_bids_eyetrack(empty_path)

    no_subject, _ = _write_record(
        tmp_path,
        "task-demo_recording-eye1_physio.tsv",
    )
    with pytest.raises(ValueError, match="participant"):
        read_bids_eyetrack(no_subject)

    explicit = read_bids_eyetrack(no_subject, participant_id="anonymous-1")
    assert set(explicit.study.data["participant"]) == {"anonymous-1"}


def test_bids_reader_validates_numeric_sidecar_fields(tmp_path):
    bad_frequency = _metadata(SamplingFrequency="1000")
    path, _ = _write_record(
        tmp_path,
        "sub-01_task-demo_recording-eye1_physio.tsv.gz",
        metadata=bad_frequency,
    )
    with pytest.raises(TypeError, match="SamplingFrequency"):
        read_bids_eyetrack(path)

    zero_frequency = _metadata(SamplingFrequency=0)
    path2, _ = _write_record(
        tmp_path,
        "sub-02_task-demo_recording-eye1_physio.tsv.gz",
        metadata=zero_frequency,
    )
    with pytest.raises(ValueError, match="positive"):
        read_bids_eyetrack(path2)

    bad_start = _metadata(StartTime="0")
    path3, _ = _write_record(
        tmp_path,
        "sub-03_task-demo_recording-eye1_physio.tsv.gz",
        metadata=bad_start,
    )
    with pytest.raises(TypeError, match="StartTime"):
        read_bids_eyetrack(path3)
