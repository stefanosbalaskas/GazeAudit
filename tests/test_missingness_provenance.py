...
def test_injection_respects_preexisting_missingness_and_zero_fraction():
    ...
    zero = inject_missingness(study, 0.0, rng=1)
    assert summarize_missingness(zero)["n_missing"] == 1

    more = inject_missingness(study, 0.5, rng=1)
    assert summarize_missingness(more)["n_missing"] == 5
...
def test_missingness_sensitivity_curve_reports_realized_loss_and_endpoint():
    ...
    assert set(first["mechanism"]) == {"mcar"}
...
def test_missingness_validation_guardrails():
    ...
    with pytest.raises(ValueError, match="between 0 and 1"):
        inject_missingness(study, np.inf)
...
def test_specification_manifest_is_stable_and_sensitive_to_decisions():
    ...
    assert first == second
    assert first["schema"] == "gazeaudit-specification-v1"
    assert first["specification_fingerprint"] != changed["specification_fingerprint"]
    assert len(first["manifest_fingerprint"]) == 64


def test_results_manifest_separates_records_fingerprint_from_environment():
    ...
    assert first == second
    assert first["schema"] == "gazeaudit-results-v1"
    ...
    changed_manifest = results_manifest(changed)
    assert first["records_fingerprint"] != changed_manifest["records_fingerprint"]
...
def test_provenance_validation_and_environment():
    ...
    assert {"gazeaudit", "python", "numpy", "pandas", "platform"}.issubset(env)
