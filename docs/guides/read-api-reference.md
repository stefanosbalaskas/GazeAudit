...
2. **generated source metadata** from the installed GazeAudit package: signature, callable kind, module, source file/line, docstring summary, import statement, and pathway backlinks.
...
- **copy import** — the exact public import statement.
...
The generated card exposes the smallest safe reusable snippet:
...
Use [Source-level API inspection]({{ '/docs/examples/source-api-inspection/' | relative_url }}) to trace `GazeStudy`, `run_specs`, and `aoi_probabilities` from generated signature metadata into their source and governed research context.
