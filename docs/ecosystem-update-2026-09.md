...
This is an ecosystem note only. It does not change GazeAudit's audit logic, release record, validation evidence or scientific claims. The gpbiometricspy addition is a separate modelling path for continuous repeated outcomes with crossed participant/item heterogeneity.
...
GazeAudit currently has repository documentation but no root MkDocs site configuration; this page updates the GitHub documentation source without inventing a new Pages/deployment pipeline.
