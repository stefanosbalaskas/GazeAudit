...
| what fields the documentation-side mapping record contains | [Data-mapping JSON Schema]({{ '/assets/data-mapping-record.schema.json' | relative_url }}) | machine-readable structure of `gazeaudit-data-mapping-record-v1`, not scientific validity |
...
| what fields define a specification declaration | [Specification declaration reference]({{ '/assets/specification-declaration-reference.json' | relative_url }}) | machine-readable robustness-question, timing, validity, failure-policy, and interpretation fields |
| what each robustness diagnostic can and cannot support | [Robustness diagnostic reference]({{ '/assets/robustness-diagnostic-reference.json' | relative_url }}) | machine-readable denominator, interpretation, plot, and reporting boundaries |
| what fields define a conclusion-recovery rule | [Conclusion rule reference]({{ '/assets/conclusion-rule-reference.json' | relative_url }}) | machine-readable reference, tolerance, sign, recovery-threshold, timing, and interpretation fields |
| what sampling/missingness perturbation helpers actually do | [Sampling & missingness reference]({{ '/assets/sampling-missingness-reference.json' | relative_url }}) | machine-readable operation, randomness, preservation, use, and interpretation boundaries |
| what execution/recovery state means in the audit ledger | [Execution-state reference]({{ '/assets/execution-state-reference.json' | relative_url }}) | machine-readable documentation-side state, denominator, evidence, next-step, and reporting contracts |
...
The [API pathways]({{ '/docs/reference/api-pathways/' | relative_url }}) page is the contextual entry point when you already know a symbol. It gives that symbol a stable deep link and connects its governed method family to the corresponding guide, runnable example, visual, and evidence boundary. When JavaScript is available, the same page adds CI-verified source metadata: inspected signatures, modules, source lines, summaries, imports, and pathway backlinks. Use [Read the source-level API reference]({{ '/docs/guides/read-api-reference/' | relative_url }}) for the interpretation rules.
