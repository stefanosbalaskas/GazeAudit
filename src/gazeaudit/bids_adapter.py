...
def read_bids_eyetrack(
    ...
    *,
    ...
    sidecar = Path(sidecar_path) if sidecar_path is not None else _infer_sidecar_path(source)
    ...
    if participant is None:
        ...
        if subject is None:
            raise ValueError(
                "could not infer participant from BIDS filename; provide participant_id explicitly"
            )
        participant = f"sub-{subject}"

    canonical_trial = trial_id if trial_id is not None else _default_trial_id(source, entities)
    output = pd.DataFrame(
        {
            "x": x,
            "y": y,
...
def read_bids_eyetrack_many(
    ...
    *,
    ...
    participant_values = _optional_sequence(participant_ids, len(paths), "participant_ids")
    ...
    for index, path in enumerate(paths):
        record = read_bids_eyetrack(
            path,
            participant_id=None if participant_values is None else participant_values[index],
...
def parse_bids_entities(filename: str) -> dict[str, str]:
    """Parse BIDS ``key-value`` entities from a filename without guessing semantics."""
...
def _validate_sidecar(metadata: Mapping[str, Any]) -> None:
    ...
    if missing:
        raise ValueError(f"BIDS eye-tracking sidecar is missing required fields: {missing}")
    ...
    if not isinstance(sampling_frequency, (int, float)) or not np.isfinite(sampling_frequency):
...
def _column_units(metadata: Mapping[str, Any], column: str, *, required: bool) -> str:
    column_metadata = metadata.get(column)
    if not isinstance(column_metadata, Mapping):
        if required:
            raise ValueError(f"BIDS sidecar must define metadata for column {column!r}")
        return ""
    units = column_metadata.get("Units")
    if not isinstance(units, str) or not units.strip():
        if required:
            raise ValueError(f"BIDS column {column!r} must define non-empty Units")
        return ""
    return units.strip()
...
def _time_factor_to_ms(unit: str) -> float:
    ...
    try:
        ...
        raise ValueError(
            f"unsupported BIDS eye-tracking timestamp unit {unit!r}; use s, ms, or us/µs"
        ) from exc
